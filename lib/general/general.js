...
module.exports = function (data, stats, uuid) {
    ...
    const name_history = getNameHistory(uuid);

    const username = player.displayname || null;
    const level = ILeveling.getTrueLevel(player.networkExp, player.networkLevel) || 0;
    ...
    if (typeof player.prefix === "undefined") {
        ...
        prefix = util.betterFormatting(util.hypixelFormattingIsWeird(player.prefix));
    }
    ...
    const gifts_sent = gifts.giftsGiven || 0;
    const gifts_received = gifts.bundlesReceived || 0;

    const links = social.links || {};

    const twitter = links.TWITTER || (social.TWITTER) ? "https://twitter.com/" + social.TWITTER : null;
    const youtube = links.YOUTUBE || (social.YOUTUBE) ? ("https://youtube.com/c/" + social.YOUTUBE).replace("0;","").replace("1;","") : null;
    const instagram = links.INSTAGRAM || (social.INSTAGRAM) ? "https://www.instagram.com/" + social.INSTAGRAM : null;
    const twitch = links.TWITCH || (social.TWITCH) ? "https://twitch.tv/" + social.TWITCH : null;
    const mixer = links.MIXER || (social.BEAM) ? "https://mixer.com/" + social.BEAM : null;
