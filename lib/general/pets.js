...
companionNames = ["FROG", "HP8", "SLOTH", "GIFTERINO", "PENGUIN", "GORRILA", "DUCK", "BLACK_PUG", "TURTLE", "WHITE_PUG", "SHIBE", "CHIMP", "PANDA", "ELEPHANT", "JACK_DOG"];

const pet_xp = constants.pet_xp;
...
module.exports = function (data, uuid) {
    ...
    for (let key in pets) {
        if (pets.hasOwnProperty(key)) {
            if (!companionNames.includes(key)) {
                xp = pets[key].experience || 0;
